...
[.text-justify]
Utilizando o programa pixels.py como referência, foi implementado um programa <<regions,regions.py>> O objetivo do programa é exibir uma determinada parte com o negativo da imagem fornecida, isto é, inverter as cores da região. Para isso, deverá ser solictado ao usuário que ele submeta coordenadas de dois pontos P1 e P2 localizados dentro dos limites do tamanho da imagem, sendo essa região exibida com o negativo da imagem.

[#regions.py]
.Código Fonte regions.py
[source,regions]
----
import cv2
import numpy as np
...
[.text-justify]
Utilizando o programa pixels.py como referência, foi implementado um programa <<trocaregioes,trocaregioes.py>> cujo objetivo é trocar os quadrantes em diagonal na imagem. 

[#trocaregioes.py]
.Código Fonte trocaregioes.py
[source,trocaregioes]
----
import cv2
import sys
...
[.text-justify]
Utilizando o programa esteg-encode.py como referência, foi implementado um programa <<decodificacao,decodificacao.py>>. O objetivo do programa é recuperar a imagem codificada de uma imagem resultante de esteganografia. Para isso, será utilizada a imagem codificada abaixo.
...
=== 5.1 Algoritmo labeling.py
[.text-justify]
Com base no programa labeling.py, foi desenvolvido um programa <<preencheregioes,preencheregioes.py>> com o objetivo de:
...
[#preencheregioes.py]
.Código Fonte preencheregioes.py
...
=== 6.1 Algoritmo equalize.py
[.text-justify]
Neste exercício, foi desenvolvido o código <<equalize,equalize.py>> tendo como base o programa histogram.py. O objetivo da atividade foi realizar a equalização do histograma de uma imagem capturada pela câmera do computador.

[#equalize.py]
.Código Fonte equalize.py
[source,equalize]
----
import cv2
import numpy as np
...
=== 6.2 Algoritmo motiondetector.py

== 7. Filtragem no domínio espacial I
=== 7.1 Filtro laplaciano do gaussiano
[.text-justify]
Tendo o programa filtroespacial.py como referência, foi implementado o programa <<laplauss,laplauss.py>> com o objetivo de calcular o laplaciano do gaussiano de imagens capturadas na câmera do computador.

[#laplauss.py]
.Código Fonte laplauss.py
[source,laplauss]
----
...
=== 8.1 Algoritmo tiltshift.py
[.text-justify]
Com base no programa addweighted.py, foi implementado o programa <<tiltshift,tiltshift.py>> de modo a implementar três ajustes na tela de interface:
